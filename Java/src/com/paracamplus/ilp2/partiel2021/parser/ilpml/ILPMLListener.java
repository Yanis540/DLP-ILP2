package com.paracamplus.ilp2.partiel2021.parser.ilpml;
...
// import antlr4.ILPgrammarPartiel2021;
import static antlr4.ILPgrammarPartiel2021Parser.*;
import antlr4.ILPgrammarPartiel2021Listener;
import antlr4.ILPgrammarPartiel2021Parser.BindingRangeContext;

public class ILPMLListener implements ILPgrammarPartiel2021Listener {
	...
	@Override
	public void enterBindingRange(BindingRangeContext ctx) {}


	@Override
	public void exitBindingRange(BindingRangeContext ctx) {
		ctx.node = factory.newRangeBlock(
			toBindings(ctx.vars, ctx.vals),
			ctx.body.node,
			toRangeBindings(ctx.vars,ctx.lows,ctx.highs)
		);
	}
	protected IASTrangeblock.IASTrangebinding[] toRangeBindings(
			List<Token> vars,
			List<ExprContext> lows, 
			List<ExprContext> highs) {
		assert(lows.size() == highs.size());
		IASTrangeblock.IASTrangebinding[] r = new IASTrangeblock.IASTrangebinding[highs.size()];
		int pos = 0;
		for (ExprContext low : lows) {
			r[pos] = factory.newRangeBinding(
				factory.newVariable(vars.get(pos).getText()),
				low.node,
				highs.get(pos).node
			);
			pos++;
