package com.paracamplus.ilp2.partiel2021_2.parser.ilpml;
...
import antlr4.ILPMLgrammarPartiel2021_2Listener;
import antlr4.ILPMLgrammarPartiel2021_2Parser.*;

public class ILPMLListener implements ILPMLgrammarPartiel2021_2Listener {
    ...
    public void exitBindingRange(BindingRangeContext ctx) {
        ctx.node = factory.newBlockRange(
            toRangeBindings(ctx.vars, ctx.vals, ctx.lows, ctx.highs), 
            ctx.body.node
        );
    }
    protected IASTblockrange.IASTbindingrange[] toRangeBindings(
			List<Token> vars, 
			List<ExprContext> exprs,
			List<ExprContext> lows,
			List<ExprContext> highs
            ) {
		if (vars == null) return new IASTblockrange.IASTbindingrange[0];
		/* par construction, vars et ctxs ont la même taille */
		assert(vars.size() == exprs.size());
		assert(vars.size() == lows.size());
		assert(vars.size() == highs.size());
		IASTblockrange.IASTbindingrange[] r = new IASTblockrange.IASTbindingrange[exprs.size()];
		int pos = 0;
		for (Token v : vars) {
			r[pos] = factory.newBindingRange(
